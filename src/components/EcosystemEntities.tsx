            ...
            return (
              ...
              >
                <div>
                  ...
                  {/* Role subtitle */}
                  <p className={`text-xs font-semibold mb-2 ${isDark ? 'text-[#FAF3EB]' : 'text-[#1C1810]'}`}>
                    {displayRole}
                  </p>

                  {/* Short description */}
                  <p className={`text-xs leading-relaxed mb-4 ${isDark ? 'text-[#D8CCA8]' : 'text-[#61502C]'}`}>
                    {displayShortDesc}
                  </p>

                  {/* Responsibilities list snippet */}
                  <div className={`space-y-2 mb-6 text-xs ${isDark ? 'text-[#FAF3EB]' : 'text-[#1C1810]'}`}>
                ...
                <div className={`pt-4 border-t flex items-center justify-between gap-3 ${
                  ...
                  <button
                    ...
                    className={`text-xs font-bold transition-colors flex items-center gap-1 ${
                      isDark ? 'text-[#D4AF37] hover:text-white' : 'text-[#B8860B] hover:text-[#1C1810]'
                    }`}
                  >
                  ...
                  {entity.link ? (
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 text-xs font-semibold rounded transition-colors ${
                        isDark ? 'bg-[#100D06] hover:bg-[#362A10] text-[#FAF3EB]' : 'bg-[#F2EAE0] hover:bg-[#E8DEC8] text-[#1C1810]'
                      }`}
                    >
        ...
        {selectedEntity && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            <div className={`relative w-full max-w-3xl border rounded-xl shadow-2xl overflow-hidden animate-fadeIn ${
              isDark ? 'bg-[#100D06] text-[#FAF3EB] border-[#4A3B18]' : 'bg-[#FFFFFF] text-[#1C1810] border-[#E2D5B8]'
            }`}>
              ...
              <div className={`flex items-center justify-between px-6 py-4 border-b ${
                ...
                <button
                  ...
                  className={`p-1.5 rounded-lg ${isDark ? 'text-[#D8CCA8] hover:text-white' : 'text-[#61502C] hover:text-black'}`}
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="p-6 space-y-6 max-h-[75vh] overflow-y-auto">
                <div>
                  ...
                  <p className={`text-xs sm:text-sm leading-relaxed ${isDark ? 'text-[#D8CCA8]' : 'text-[#423414]'}`}>
                  ...
                  <div className="grid grid-cols-1 gap-2 text-xs">
                    {(isTa && selectedEntity.taKeyResponsibilities ? selectedEntity.taKeyResponsibilities : selectedEntity.keyResponsibilities).map((resp, i) => (
                      <div key={i} className={`flex items-start gap-2.5 p-2.5 border rounded ${
                        isDark ? 'bg-[#241E0F] border-[#4A3B18] text-[#FAF3EB]' : 'bg-[#FAF7F2] border-[#E2D5B8] text-[#1C1810]'
                      }`}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className={`p-4 border rounded ${
                    ...
                    <h5 className={`text-xs font-semibold mb-2 ${isDark ? 'text-white' : 'text-[#1C1810]'}`}>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {(isTa && selectedEntity.taOperationalFocus ? selectedEntity.taOperationalFocus : selectedEntity.operationalFocus).map((focus, i) => (
                        <span key={i} className={`px-2 py-0.5 text-[11px] rounded border ${
                          isDark ? 'bg-[#100D06] text-[#D4AF37] border-[#4A3B18]' : 'bg-[#FFFFFF] text-[#B8860B] border-[#E2D5B8]'
                        }`}>
                          {focus}
                        </span>
                      ))}
                  ...
                  <div className={`p-4 border rounded ${
                    ...
                    <h5 className={`text-xs font-semibold mb-1 ${isDark ? 'text-white' : 'text-[#1C1810]'}`}>
