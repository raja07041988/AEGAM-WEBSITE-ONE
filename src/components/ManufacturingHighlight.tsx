        ...
        <div className="mt-12 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          ...
          <div className="lg:col-span-6 relative">
            <div className={`rounded-2xl overflow-hidden border shadow-2xl relative group ${
              ...
              <div className={`absolute bottom-4 left-4 right-4 p-4 backdrop-blur-md rounded-xl border flex items-center justify-between ${
                isDark ? 'bg-[#150709]/90 border-[#4D1E27]' : 'bg-[#FFFFFF]/90 border-[#E2D5B8]'
              }`}>
                <div>
                  ...
                  <p className={`text-xs font-bold ${isDark ? 'text-white' : 'text-[#2B1015]'}`}>{name}</p>
                </div>
                <button
          ...
          <div className="lg:col-span-6 space-y-6">
            ...
            <div>
              ...
              <h3 className={`text-xl sm:text-2xl font-bold ${isDark ? 'text-white' : 'text-[#2B1015]'}`}>
                {tagline}
              </h3>
              <p className={`text-xs sm:text-sm mt-3 leading-relaxed ${isDark ? 'text-[#D1B8BC]' : 'text-[#5C2B32]'}`}>
                {description}
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 pt-2">
              <div className={`p-3 border rounded-lg ${
                ...
                <span className={`text-[10px] block uppercase tracking-wider ${isDark ? 'text-[#D1B8BC]' : 'text-[#704249]'}`}>
            ...
            {/* Features checkmarks */}
            <div className={`space-y-2 text-xs ${isDark ? 'text-[#D1B8BC]' : 'text-[#4A1E24]'}`}>
            ...
            <div className="pt-4 flex flex-wrap items-center gap-3">
              ...
              <button
                ...
                className={`px-5 py-2.5 border font-semibold text-xs rounded transition-colors flex items-center gap-1.5 ${
                  isDark 
                    ? 'bg-[#2E1218] hover:bg-[#36151C] text-[#FAF3EB] border-[#4D1E27]' 
                    : 'bg-[#F2EAE0] hover:bg-[#E8DEC8] text-[#2B1015] border-[#E2D5B8]'
                }`}
              >
        ...
        <div className={`mt-16 p-6 border rounded-xl flex flex-col md:flex-row items-center justify-between gap-4 ${
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className={`text-xs mt-0.5 ${isDark ? 'text-[#D1B8BC]' : 'text-[#5C2B32]'}`}>
                {isTa ? (
          ...
          <a
            ...
            className={`shrink-0 px-4 py-2 border text-xs font-semibold rounded transition-colors flex items-center gap-1.5 ${
              isDark 
                ...
                : 'border-[#8C2A38]/40 hover:border-[#8C2A38] text-[#8C2A38]'
            }`}
          >
