    ...
    <div className={`min-h-screen py-12 lg:py-20 transition-colors duration-300 ${
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
        ...
        <div className="max-w-3xl space-y-4">
          ...
          <p className={`text-base leading-relaxed ${isDark ? 'text-[#D8CCA8]' : 'text-[#61502C]'}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-4 space-y-3">
              ...
              return (
                <button
                  ...
                  className={`w-full text-left p-4 rounded-xl border transition-all flex items-center justify-between group ${
                    isActive
                      ? isDark
                        ? 'bg-[#241E0F] text-white border-[#C5A059] shadow-lg'
                        : 'bg-[#B8860B] text-white border-[#B8860B] shadow-lg'
                      : isDark
                        ? 'bg-[#100D06] text-[#FAF3EB] border-[#4A3B18] hover:border-[#C5A059]/50'
                        : 'bg-white text-[#1C1810] border-[#E2D5B8] hover:border-[#B8860B]/50'
                  }`}
                >
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      <span className={`font-bold text-sm ${isActive ? 'text-white' : (isDark ? 'text-white' : 'text-[#1C1810]')}`}>
                        {displayName}
                      </span>
                    </div>
                    <p className={`text-xs ${isActive ? 'text-white/80' : (isDark ? 'text-[#D8CCA8]' : 'text-[#61502C]')}`}>
                      {displayType}
                    </p>
                  </div>
            ...
            <div className={`p-5 border rounded-xl space-y-3 mt-6 ${
              isDark ? 'bg-[#100D06] border-[#4A3B18]' : 'bg-[#1C1810] text-white border-[#423414]'
            }`}>
              ...
              <p className="text-xs text-[#D8CCA8] leading-relaxed">
              ...
              <button
                ...
                className={`w-full py-2 border rounded text-xs font-bold transition-colors ${
                  isDark 
                    ? 'bg-[#241E0F] hover:bg-[#362A10] text-[#D4AF37] border-[#4A3B18]' 
          ...
          <div className={`lg:col-span-8 border rounded-2xl p-6 sm:p-10 shadow-sm space-y-8 ${
            ...
            <div className={`flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b ${
              ...
              <div>
                ...
                <h2 className={`text-2xl font-extrabold ${isDark ? 'text-white' : 'text-[#1C1810]'}`}>
                  {name}
                </h2>
              </div>
              ...
              {selectedEntity.link ? (
                <a
                  ...
                  className={`px-4 py-2 font-bold text-xs rounded-lg transition-colors inline-flex items-center gap-1.5 ${
                    isDark ? 'bg-[#100D06] text-[#D4AF37] hover:bg-[#362A10]' : 'bg-[#B8860B] text-white hover:bg-[#A37409]'
                  }`}
                >
            ...
            <div>
              ...
              <p className={`text-sm font-semibold ${isDark ? 'text-white' : 'text-[#1C1810]'}`}>{role}</p>
              <p className={`text-xs sm:text-sm mt-2 leading-relaxed ${isDark ? 'text-[#D8CCA8]' : 'text-[#61502C]'}`}>
                {fullDesc}
              </p>
            </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                {keyResps.map((resp, i) => (
                  <div key={i} className={`p-3 border rounded-lg flex items-start gap-2.5 ${
                    isDark ? 'bg-[#100D06] border-[#4A3B18] text-[#FAF3EB]' : 'bg-[#FAF7F2] border-[#E2D5B8] text-[#1C1810]'
                  }`}>
            ...
            <div className={`grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t ${
              ...
              <div className={`p-4 rounded-xl border ${
                ...
                <h4 className={`text-xs font-bold mb-2 ${isDark ? 'text-white' : 'text-[#1C1810]'}`}>{t.ecosystemPage.focusTags}</h4>
                <div className="flex flex-wrap gap-1.5">
                  {focus.map((f, i) => (
                    <span key={i} className={`px-2.5 py-1 text-[11px] font-semibold rounded border ${
                      isDark ? 'bg-[#241E0F] text-[#D4AF37] border-[#4A3B18]' : 'bg-white text-[#B8860B] border-[#E2D5B8]'
                    }`}>
                      {f}
                    </span>
                  ))}
              ...
              <div className={`p-4 rounded-xl border ${
                ...
                <h4 className={`text-xs font-bold mb-1 ${isDark ? 'text-white' : 'text-[#1C1810]'}`}>{t.ecosystemPage.governanceBoundary}</h4>
                <p className={`text-xs ${isDark ? 'text-[#D8CCA8]' : 'text-[#61502C]'}`}>{govScope}</p>
              </div>
            </div>
        ...
        <div className={`border rounded-2xl p-8 lg:p-12 space-y-6 ${
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-xs">
            <div className={`p-4 border rounded-xl space-y-2 ${isDark ? 'bg-[#241E0F] border-[#4A3B18]' : 'bg-[#18140B] border-[#423414]'}`}>
              <strong className="text-[#D4AF37] block font-bold">{isTa ? 'செயல்பாட்டு பங்களிப்பு' : 'Operational Contributions'}</strong>
              <p className="text-[#D8CCA8]">{isTa ? 'பொறியாளர்கள், பட்டறை ஊழியர்கள் மற்றும் தொழில்நுட்ப வல்லுநர்களின் பங்களிப்பு.' : 'Engineers, workshop contributors, and technical specialists providing expertise.'}</p>
            </div>
            <div className={`p-4 border rounded-xl space-y-2 ${isDark ? 'bg-[#241E0F] border-[#4A3B18]' : 'bg-[#18140B] border-[#423414]'}`}>
              <strong className="text-[#D4AF37] block font-bold">{isTa ? 'டீலர்ஷிப் & விநியோகம்' : 'Dealership & Distribution'}</strong>
              <p className="text-[#D8CCA8]">{isTa ? 'ரோட்டரி எண்ணெய் பிழிந்தெடுக்கும் இயந்திரங்கள் மற்றும் உதிரிபாகங்களை விநியோகிக்கும் கூட்டாளர்கள்.' : 'Authorized regional partners distributing rotary oil machinery and spare parts.'}</p>
            </div>
            <div className={`p-4 border rounded-xl space-y-2 ${isDark ? 'bg-[#241E0F] border-[#4A3B18]' : 'bg-[#18140B] border-[#423414]'}`}>
              <strong className="text-[#D4AF37] block font-bold">{isTa ? 'மூலோபாய ஒத்துழைப்பு' : 'Strategic Collaboration'}</strong>
              <p className="text-[#D8CCA8]">{isTa ? 'தொழில்துறை மற்றும் கல்வி நிறுவனங்களுடன் இணைந்து செயல்படும் கூட்டு.' : 'Industrial, academic, and technology collaborators aligned with ecosystem goals.'}</p>
            </div>
            <div className={`p-4 border rounded-xl space-y-2 ${isDark ? 'bg-[#241E0F] border-[#4A3B18]' : 'bg-[#18140B] border-[#423414]'}`}>
              <strong className="text-[#D4AF37] block font-bold">{isTa ? 'சுற்றுச்சூழல் வழிகாட்டுதல்' : 'Ecosystem Stewardship'}</strong>
              <p className="text-[#D8CCA8]">{isTa ? 'நீண்ட கால நிறுவனத் தொடர்ச்சியை உறுதி செய்யும் ஏகம் டிரஸ்ட் நிர்வாகம்.' : 'Governed by AEGAM TRUST to ensure long-term institutional continuity.'}</p>
            </div>
          </div>
        </div>
